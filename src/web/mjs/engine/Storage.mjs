...
export default class Storage {
    ...
    constructor() {
        ...
        // Use local storage/ folder in the app directory for portability and crash-safety
        let appPath = electron.remote.app.getAppPath();
        this.storageDir = this.path.join(appPath, 'storage');
        this._createDirectoryChain(this.storageDir);
    ...
    /**
     * Initialize SQLite (SQL.js) database for structured storage.
     * Creates the database file if it doesn't exist.
     */
    _initDatabase() {
        try {
            if (typeof SQL === 'undefined') {
                console.warn('SQL.js not available, import queue will use JSON fallback');
                return;
            }
            let exists = this.fs.existsSync(this.dbPath);
            if (exists) {
                let buffer = this.fs.readFileSync(this.dbPath);
                this._db = new SQL.Database(buffer);
            } else {
                this._db = new SQL.Database();
            }
            // Create tables if they don't exist
            this._db.run(`CREATE TABLE IF NOT EXISTS import_queue (
            ...
            this._db.run(`CREATE TABLE IF NOT EXISTS queue_state (
                key TEXT PRIMARY KEY,
                value TEXT
            )`);
            // Save the newly created tables to disk
    ...
    /**
     * Write the current SQLite database buffer to disk.
     */
    _persistDatabase() {
        try {
            if (this._db) {
    ...
    // ---- SQLite-backed Import Queue Storage ----
    ...
    /**
     * Clear the import queue data from SQLite database.
     */
